...
echo "== PyInstaller =="
# Cette construction ecrit chez elle, comme celle de Linux ecrit chez elle :
# --distpath et --workpath suffisent, et il n'y a plus rien a mettre de cote
# ni a renommer. Les mv qui vivaient ici ont coute cher -- l'un effacait le
# paquet Linux du jour, l'autre glissait la construction *dans* le dossier
# de la precedente au lieu de la remplacer.
#
...
if [ ! -f dist.windows/ZyRoom-Qt/ZyRoom-Qt.exe ]; then
...
# version de l'archive mentirait. Le garde-fou reste, meme si la cause qu'il
# guettait a disparu avec les renommages.
...
mkdir -p dist
rm -f "dist/ZyRoom-Qt-${version}-windows.zip"
